...
var Recording = function (name) {
  ...
  var advance = function(messageCallback) {
    ...
    var processNext = function(message) {
      ...
      case "INBOUND":
        ...
        return true;
      case "RULE_ADD":
        console.log("Adding rule for " + message.condition);
        rules = rules.push({condition: message.condition,
  ...
  var advancePast = function(messageToMatch, messageCallback) {
    rules.forEach(function(rule) {
      if (rule.condition === messageToMatch) {
        console.log("Applying rule");
        rule.application(messageCallback, messageToMatch);
      } else {
        console.log("Rule does not match: " + rule.condition);
      }
      return false;
    });
