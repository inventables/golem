...
var Recording = function (name) {
    ...
    var org = { inventables:
                { golem: {
                  ...
                  expect: function(matcher, onMatch) {
                    ret = ret.push({type: "OUTBOUND", matches: function(toMatch) {
                      return toMatch == matcher.value;
                    }});
  ...
  var outboundMessageMatches = function(outboundEvent, messageToMatch) {
    return outboundEvent.matches(messageToMatch);
  };
  ...
  var advancePast = function(messageToMatch, messageCallback) {
    ...
    if (readPos < eventStream.size && outboundMessageMatches(eventStream.get(readPos), messageToMatch)) {
      readPos++;
